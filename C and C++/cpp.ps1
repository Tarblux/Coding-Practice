...
function ConfirmBaseRun {
  ...
  if ($isWingetInstalled -eq "not-installed" -or $isChocoInstalled -eq "not-installed") {
  ...
  # Optional: if winget is "outdated", warn but continue
  if ($isWingetInstalled -eq "outdated") {
    Write-Warning "Winget appears older than the latest GitHub release, but is installed. Continuing."
  }
}
...
Start-Process -NoNewWindow -Wait winget -ArgumentList 'install','ezwinports.make'
